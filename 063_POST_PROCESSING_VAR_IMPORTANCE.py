...
data_top20 = data_importance[ data_importance.ranking < 20 ].round(decimals = 2)

table = data_top20.transpose()
table.columns = table[table.index == 'Variable'].values.tolist()[0]
#table[ table.index == 'Model'].values.tolist()[0]
table = table.ix[[1,2,3,4], : ]
...
# create plot
fig, ax = plt.subplots(); index = np.arange(19); bar_width = 0.35; opacity = 0.8
plt.bar(index, data_top20.VI_ALL, bar_width, alpha=opacity, color='b', label='ALL')
# plt.bar(index + bar_width, data_top20.VI_multi, bar_width, alpha=opacity, color='g', label='MULTI')
