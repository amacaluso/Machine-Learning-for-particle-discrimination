...
data_enet = data[data$Method == 'E_NET', ]

# Get the dataset.
input <- data_enet[ , c('Accuracy', 'n_variables', 'Model') ]

# Create the regression model.
result1 <- aov(Accuracy ~ n_variables * Model, data = input)
result2 <- aov(Accuracy ~ n_variables + Model, data = input)
...
ggplot(data = data, aes(x = n_variables, y = Accuracy)) + geom_point( aes( color = Model) ) + geom_line( aes( color = Model) ) + facet_wrap(~ Method,  nrow=3, scale = "free") + ggtitle('Model Comparison')
